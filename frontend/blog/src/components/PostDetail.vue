<script setup>
import { ref, watch } from 'vue'; // defineProbs
...
// const title = ref(postData.postById.title);
// const text = ref(postData.postById.text);
// const category = ref(postData.postById.category.name);
// const owner = ref(postData.postById.owner.firstName + ' ' + postData.postById.owner.lastName);
// const tags = ref(postData.postById.tags);
// const imageURL = ref(process.env.VUE_APP_MEDIA_URL + postData.postById.image);
//
// let options = { weekday: 'long', year: 'numeric', month: '2-digit', day: '2-digit' };
// const date = ref(new Date(postData.postById.dateCreated).toLocaleDateString("en-GB", options));
